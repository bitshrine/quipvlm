...
def eval(predictions,
         results_file = 'gqa/results.txt',
         tier: str = "val",
         scenes: str = "{tier}_sceneGraphs.json", questions: str = "{tier}_all_questions.json",
         choices: str = "{tier}_choices.json",
         attentions: str = "{tier}_attentions.json", 
         consistency = False,
         grounding = False,
    ...
    def loadFile(answer_file):
        # load standard json file
        if os.path.isfile(answer_file):
            with open(answer_file) as file:
                data = json.load(file)
        # load file chunks if too big 
        elif os.path.isdir(answer_file.split(".")[0]):
            data = {}
            chunks = glob.glob('{dir}/{dir}_*.{ext}'.format(dir = answer_file.split(".")[0], ext = answer_file.split(".")[1]))
            for chunk in chunks:
                with open(chunk) as file: 
                    data.update(json.load(file))
        else:
            raise Exception("Can't find {}".format(answer_file))
        return data

    # Load scene graphs
    print("Loading scene graphs...")
    scenes = loadFile(scenes.format(tier = tier))

    # Load questions
    print("Loading questions...")
    questions = loadFile(questions.format(tier = tier))

    # Load choices
    print("Loading choices...")
    choices = loadFile(choices.format(tier = tier))

    # Load predictions and turn them into a dictionary
    print("Loading predictions...")
    predictions = loadFile(predictions.format(tier = tier))
    predictions = {p["questionId"]: p["prediction"] for p in predictions}

    # Make sure all question have predictions
    for qid in questions:
        if (qid not in predictions) and (consistency or questions[qid]["isBalanced"]):
    ...
    def computeGroundingScore(question, sceneGraph, attentionMap):
        ...
        if objectFeatures:
            ...
            cells = [(getCell(i, j), attentionMap[i][j]) for i in range(mapSize) for j in range(mapSize)]
    ...
    # print / output
    with open(results_file, 'a+', 1) as f_out:
        s = ""
        print()
        f_out.write(s + '\n')
        for m in metrics:
            # skip grounding and consistency scores if not requested
            if m == "grounding" and not grounding:
                continue
            if m == "consistency" and not consistency:
                continue

            # print score
            s = "{title}: {score:.2f}{suffix}".format(title = m.capitalize(), score = scores[m], 
        ...
        for m, mPrintName in detailedMetrics:
            ...
            for t in sorted(list(scores[m].keys())):
                ...
                # print score
                s = "  {title}: {score:.2f}{suffix} ({amount} questions)".format(title = tName, 
