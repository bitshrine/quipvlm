...
class QuantMethod:
    ...
    def preproc(self, preproc_gptqH=False, percdamp=.01,
        ...
        if preproc_rescale:
            ...
            # We need to reshape the weight like gptq does
            if isinstance(self.layer, nn.Conv2d):
                w = w.flatten(1)
            if isinstance(self.layer, transformers.Conv1D):
                w = w.t()
            ...
            # Restore the weight's original shape
            if isinstance(self.layer, nn.Conv2d):
                w = w.reshape(self.layer.weight.data.shape)
            if isinstance(self.layer, transformers.Conv1D):
                w = w.t()
    ...
    def error_compute(self, full_W, quant_W):
        full_W = full_W.float()
        quant_W = quant_W.float().reshape(full_W.shape)
